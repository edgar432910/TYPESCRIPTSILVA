...
export class AuthUseCase implements AuthRepository{
    constructor( private repositoryUser:UserRepository, private repositoryFamilyRefreshTokens:FamilyRefreshTokensRepository){

    }

   async login(user:Partial<UserModel>):Promise<Result<TokensModel>>{

      const result:Result<UserModel> = await this.repositoryUser.getOne({email:user.email}, ["roles","familyRefreshTokens"]);

      const userMatched:UserModel = result.payload.data as UserModel;

      if(userMatched){

        const isPasswordValid = UserService.decryptPassword(user.password, userMatched.password);
        if(isPasswordValid){

            const newValue = UserService.generateRefreshToken();
            // da de baja el id
           await this.repositoryFamilyRefreshTokens.update({status:false},{user:userMatched.id},[]) 
            // fecha de expiracion 
           /*  if(compareAsc(new Date(), userMatched.dateExpirationRefreshToken)>0){
            ...
            const newModelRT:FamilyRefreshTokensModel ={refreshToken:newValue, user:userMatched}
   ...
   async getNewAccessToken(refreshToken:string):Promise<Result<TokensModel>>{
        ...
        const result:Result<FamilyRefreshTokensModel>  = await this.repositoryFamilyRefreshTokens.getOne({refreshToken, status:true}
            , ["user"]);
        ...
        if(refreshTokenMatched){
            ...
            const user:Result<UserModel> = await this.repositoryUser.getOne({id:refreshTokenMatched.user.id},["familyRefreshTokens"]);
            
           
            const newValue = UserService.generateRefreshToken();

           const newModelRT:FamilyRefreshTokensModel ={refreshToken:newValue, user:refreshTokenMatched.user}
